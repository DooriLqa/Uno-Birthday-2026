...
- квиз содержит 20 вопросов и требует 5 правильных ответов подряд;
- история увиденных/решённых вопросов сохраняется отдельно, поэтому новые вопросы получают приоритет;
- аудио радио живёт вне React-компонента и продолжает играть после закрытия модального окна;
- фоновые радиопомехи генерируются через Web Audio API и становятся тише возле заданных частот.

Для реальных записей создайте `public/audio/radio/` и положите туда:
`station-01.mp3` ... `station-08.mp3`.
