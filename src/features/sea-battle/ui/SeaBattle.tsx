  ...
  const fire = useCallback(() => {
    ...
    cooldownTimerRef.current = window.setInterval(() => {
      const remaining = Math.max(
        0,
        cooldownUntilRef.current - performance.now(),
      )
  ...
  return (
    <div className="sea-battle">
      <div className="sea-battle__hud">
        <div><span>Попадания</span><strong>{hits}/{REQUIRED_HITS}</strong></div>
        <div><span>Торпеды</span><strong>{torpedoesLeft}/{START_TORPEDOES}</strong></div>
        <div><span>Перезарядка</span><strong>{cooldown > 0 ? `${cooldownSeconds} сек` : 'ГОТОВ'}</strong></div>
      </div>
      ...
      <div className="sea-battle__controls">
        <button type="button" onClick={() => moveCamera(-70)} disabled={!started}>←</button>
        <button type="button" className="sea-battle__fire" onClick={fire} disabled={!started || cooldown > 0 || torpedoesLeft <= 0}>ПУСК</button>
        <button type="button" onClick={() => moveCamera(70)} disabled={!started}>→</button>
      </div>
      ...
      {!started && (
        <div className="sea-battle__overlay">
          <div className="sea-battle__message">
            <h3>{result === 'win' ? 'Победа!' : result === 'lose' ? 'Боезапас закончился' : 'Морской бой'}</h3>
            <p>
              {result === 'win'
            ...
            <button type="button" onClick={resetGame}>{result ? 'Играть ещё раз' : 'Начать игру'}</button>
          </div>
        </div>
      )}
    </div>
