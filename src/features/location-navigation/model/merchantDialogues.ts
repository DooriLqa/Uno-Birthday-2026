...
import merchantSprite from '@/shared/assets/dialogues/shiba-merchant.png'
...
const merchant: DialogueSpeaker = {
  id: 'shiba-merchant', name: 'Пончик, торговец', sprite: merchantSprite,
}
...
export function openMapWarning() {
  openDialogue({ id: 'island-map-required', messages: [{
    id: 'warning', speaker: { id: 'yuni', name: 'Юни', isMain: true }, emotion: 'neutral',
...
function openQuizInvitation(onOpenQuiz: () => void) {
  openDialogue({ id: 'tourist-merchant-quiz', messages: [
    { id: 'invite', speaker: merchant, emotion: 'happy', text: 'Сыграем в квиз? Пять правильных ответов — и радиоприёмник твой. Попытка стоит одну монетку.' },
    { id: 'start', speaker: merchant, emotion: 'happy', text: 'Приступим! Правила и кнопка начала — на следующем экране.', onComplete: onOpenQuiz },
  ] })
}

function buyOilLantern() {
  if (hasOilLantern()) {
    openDialogue({ id: 'tourist-merchant-lantern-owned', messages: [{
      id: 'owned', speaker: merchant, emotion: 'happy',
  ...
  if (!usePawCoinStore.getState().spendPawCoins(LANTERN_COST)) {
    openDialogue({ id: 'tourist-merchant-lantern-poor', messages: [{
      id: 'not-enough', speaker: merchant, emotion: 'neutral',
  ...
  openDialogue({ id: 'tourist-merchant-lantern-bought', messages: [{
    id: 'bought', speaker: merchant, emotion: 'happy',
...
function openMerchantOptions(onOpenQuiz: () => void) {
  ...
  openDialogue({ id: 'tourist-merchant-options', messages: [{
    id: 'options', speaker: merchant, emotion: 'happy',
    text: hasOilLantern() ? 'Фонарь пригодился? Чем ещё могу помочь?' : 'Для пещеры нужен надёжный свет. Что выберешь?',
    choices,
  }] })
}
...
export function talkToMerchant(onOpenQuiz: () => void) {
  ...
  if (!state.readDialogueIds.includes(GREETING_ID)) {
    openDialogue({ id: GREETING_ID, messages: [
      { id: 'welcome', speaker: merchant, emotion: 'happy',
        text: 'Добро пожаловать на остров! Я Пончик. Слева от причала — тихая бухта с библиотекой, а здесь всегда найдётся развлечение.' },
      { id: 'map', speaker: merchant, emotion: 'neutral',
        text: 'Держи карту острова. Теперь можно посмотреть, куда ведёт тропинка за причалом.',
        onComplete: () => {
          if (!hasIslandMap()) useInventoryStore.getState().addItem({ id: MAP_ITEM_ID, name: 'Карта острова', icon: '🗺️' })
        } },
      { id: 'quiz-next', speaker: merchant, emotion: 'happy',
