...
type GamePhase = 'idle' | 'showing' | 'shuffling' | 'choosing' | 'result'
...
export const ShellGame = ({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setCups((currentCups) => {
        ...
        const firstCup = currentCups.find((cup) => cup.position === firstPosition)
        const secondCup = currentCups.find((cup) => cup.position === secondPosition)
    ...
    return () => window.clearTimeout(timer)
  }, [phase, shuffleStep, shuffleIterations, shufflePause, swap])
  ...
  const visibleCups = [...cups].sort((a, b) => a.position - b.position)

  const shouldShowPearl = phase === 'showing' || (phase === 'result' && result === 'win')

  return (
    <div className="shell-game">
      <div className="shell-game__header">
        {phase === 'idle' && (
          <p className="shell-game__hint">Найди, под каким стаканом находится жемчужина</p>
        )}

        {phase === 'showing' && (
          <p className="shell-game__hint">Запомни, где находится жемчужина!</p>
        )}
        ...
        {phase === 'choosing' && <p className="shell-game__hint">Выбери стакан</p>}

        {phase === 'result' && (
          <p
            className={`shell-game__result ${
              result === 'win' ? 'shell-game__result--win' : 'shell-game__result--lose'
            }`}
          >
      ...
      <div className="shell-game__board">
        <div className="shell-game__cups">
          {visibleCups.map((cup) => {
            ...
            if (swap && isSwapping) {
              const targetPosition = isFirst ? swap.secondPosition : swap.firstPosition
            ...
            const showPearl = shouldShowPearl && pearlCupId === cup.id

            return (
              <div
                key={cup.id}
                className={`shell-game__cup-wrapper shell-game__cup-wrapper--position-${cup.position} ${
                  isSwapping ? 'shell-game__cup-wrapper--swapping' : ''
                }`}
                style={style}
              >
                <button
                  type="button"
                  className={`shell-game__cup ${isLifted ? 'shell-game__cup--lifted' : ''} ${
                    isSwapping ? 'shell-game__cup--animating' : ''
                  } ${phase === 'choosing' ? 'shell-game__cup--selectable' : ''}`}
                  onClick={() => handleCupClick(cup.id)}
                  disabled={phase !== 'choosing'}
                  aria-label={`Стакан ${cup.position + 1}`}
                >
                  {showPearl && (
                    <img className="shell-game__pearl" src={pearlImage} alt="Жемчужина" />
                  )}

                  <img className="shell-game__cup-image" src={cupImage} alt="Стакан" />
                </button>
              </div>
            )
          })}
      ...
      <div className="shell-game__controls">
        {phase === 'idle' && (
          <button type="button" className="shell-game__play-button" onClick={startGame}>
            Играть
          </button>
        )}
