...
const createDeck = (): Card[] =>
  SUITS.flatMap((suit) =>
    RANKS.map((rank) => {
      const path = Object.keys(CARD_IMAGES).find((key) =>
        new RegExp(`${suit}${rank}\\.(png|jpg|jpeg|webp)$`).test(key),
      )
      if (!path) throw new Error(`Не найдено изображение карты ${suit}${rank}`)
      return {
        id: `${suit}${rank}-${Math.random()}`,
        rank,
        image: CARD_IMAGES[path],
...
export function BlackJack({ onComplete }: BlackJackProps) {
  ...
  const finish = (nextPlayer: Card[], nextDealer: Card[]) => {
    const p = handValue(nextPlayer)
    const d = handValue(nextDealer)
    ...
    <div className="black-jack">
      <div className="black-jack__table">
        <section className="black-jack__hand">
          <div className="black-jack__hand-header">
            <h2>Раздающий</h2>
            <span>{revealed ? dealerValue : '?'} очков</span>
          </div>
          <div className="black-jack__cards">
            {dealer.map((card, i) => (
              <div className="black-jack__card" key={card.id}>
                {i === 0 && !revealed ? (
                  <div className="black-jack__card-back">♠</div>
                ) : (
      ...
      {result && (
        <p className={`black-jack__result black-jack__result--${result}`}>
          {result === 'win' ? 'Победа!' : 'Увы, ты проиграл.'}
        </p>
      )}
      <div className="black-jack__controls">
        {!started || result ? (
          <button type="button" onClick={startRound}>
            {result ? 'Следующий раунд' : 'Играть'}
          </button>
        ) : (
          <>
